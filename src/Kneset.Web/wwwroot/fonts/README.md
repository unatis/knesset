# Мультискриптовые шрифты

У этих файлов два потребителя:

- **OG-картинки.** `OgImageService` рисует превью на сервере через SixLabors и читает TTF
  прямо с диска — `Path.Combine(env.WebRootPath, "fonts", …)`.
- **Сайт.** `wwwroot/app.css` объявляет их через `@font-face` под именами
  `Title-Multiscript` и `Body-Multiscript` и ставит вторыми в стеки `--kt-font-display`
  и `--kt-font-sans`. Основные шрифты визуального слоя (Instrument Serif, Geist) глифов
  иврита и арабского не содержат, поэтому весь текст на этих письменностях рисуется отсюда.

Из-за второго потребителя папка лежит внутри `wwwroot` — иначе браузер не смог бы забрать
файлы. Отдаются они по пути `/fonts/…`.
...
Если у замены другие веса, поправить `font-weight` в двух `@font-face` в `app.css`.
