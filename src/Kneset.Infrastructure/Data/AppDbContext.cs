    ...
    : IdentityDbContext<AppUser>(options), IDataProtectionKeyContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Faction>(f =>
        {
            ...
            // только добавил бы лишний переход при связке с депутатами.
            f.Property(x => x.Id).ValueGeneratedNever();
            f.Property(x => x.NameHe).HasMaxLength(300);
            f.Property(x => x.OriginKind).HasMaxLength(16);
            f.Property(x => x.GroupKey).HasMaxLength(32);
            f.Property(x => x.Source).HasMaxLength(300);
            f.HasIndex(x => x.GroupKey);
        });

        modelBuilder.Entity<FactionParty>(p =>
        {
        ...
        {
            // Один подзаконный акт на закон — ключ приходит из источника.
            r.HasIndex(x => new { x.IsraelLawId, x.KnessetActId }).IsUnique();
            r.Property(x => x.Name).HasMaxLength(1000);
            r.Property(x => x.PublicationSeries).HasMaxLength(100);
            r.Property(x => x.MagazineNumber).HasMaxLength(50);
            r.Property(x => x.PageNumber).HasMaxLength(50);
            r.HasOne(x => x.IsraelLaw).WithMany(l => l.Regulations)
             .HasForeignKey(x => x.IsraelLawId).OnDelete(DeleteBehavior.Cascade);
        });
        ...
        {
            // Один перевод на язык. Иврит здесь не хранится — он в IsraelLaw.Name.
            t.HasIndex(x => new { x.IsraelLawId, x.LanguageCode }).IsUnique();
            t.Property(x => x.LanguageCode).HasMaxLength(8);
            t.Property(x => x.Text).HasMaxLength(2000);
            t.Property(x => x.SourceName).HasMaxLength(2000);
            t.HasOne(x => x.IsraelLaw).WithMany(l => l.Titles)
             .HasForeignKey(x => x.IsraelLawId).OnDelete(DeleteBehavior.Cascade);
        });
        ...
        {
            l.Property(x => x.SiteDataVersion).HasMaxLength(16);
